...
function run_experiment(g, θ, x0_set, expert_traj_list, parameterized_cost, 
                        ...
                        obs_time_list = 1:game_horizon-1, obs_state_list = 1:nx, obs_control_list = 1:nu, equilibrium_type = "FBNE_costate", convergence_tol=0.01, terminate_when_small_progress=true, initial_step_size=2.0, ground_truth_expert=[])
    n_data = length(x0_set)
    ...
    for iter in 1:n_data
        ...
        conv_table[iter], sol_table[iter], loss_table[iter], grad_table[iter], equi_table[iter], consistent_information_pattern, ground_truth_loss[iter] = objective_inference_with_partial_obs(x0,
                                                                        ...
                                                                        obs_time_list, obs_state_list, obs_control_list, convergence_tol, terminate_when_small_progress, initial_step_size, ground_truth_expert)

...
function belief_mean_update_policy(x,u,τ,π_P_list,π_α_list,u_list,x_list)
    ...
    return - 1/ΔT* x[8]*π_P_list[t][3:4,6]'*inv(I(2)+π_P_list[t][3:4,6]*x[8]*π_P_list[t][3:4,6]') * ( 
        [control3_policy(x,τ,π_P_list,π_α_list,u_list,x_list)+u[3] - player1_imagined_control_policy(x,τ,3,π_P_list,π_α_list,u_list,x_list);  
        control4_policy(x,τ,π_P_list,π_α_list,u_list,x_list)+u[4] - player1_imagined_control_policy(x,τ,4,π_P_list, π_α_list,u_list,x_list)] )
end
...
function active_control_policy(x,u,t,π_P_list,π_α_list,u_list,x_list)
    return [
        control1_policy(x,t,π_P_list,π_α_list,u_list,x_list); 
        control2_policy(x,t,π_P_list,π_α_list,u_list,x_list); 
        control3_policy(x,t,π_P_list,π_α_list,u_list,x_list)+u[3]; 
        control4_policy(x,t,π_P_list,π_α_list,u_list,x_list)+u[4];]
end
...
for outer_iter in 1:num_theta
    ...
    for iter in 1:num_samples
        ...
        dx(cs::player_dynamics2, x, u, t) = SVector(
            ...
            (control2_policy(x,t,π_P_list_tmp,π_α_list_tmp,u_list_tmp,x_list_tmp) - control4_policy(x,t,π_P_list_tmp,π_α_list_tmp,u_list_tmp,x_list_tmp)-u[4])*cos(x[3]) +
                (control1_policy(x,t,π_P_list_tmp,π_α_list_tmp,u_list_tmp,x_list_tmp) - control3_policy(x,t,π_P_list_tmp,π_α_list_tmp,u_list_tmp,x_list_tmp)-u[3])*sin(x[3]), # x[3] is the heading angle of the robot
            -1/ΔT*x[4] + 1/ΔT*x[1] + 1/ΔT * L*cos(x[3]) + control3_policy(x,t,π_P_list_tmp,π_α_list_tmp,u_list_tmp,x_list_tmp) + u[3], # x[4] is the x position of human
            -1/ΔT*x[5] + 1/ΔT*x[2] + 1/ΔT * L*sin(x[3]) + control4_policy(x,t,π_P_list_tmp,π_α_list_tmp,u_list_tmp,x_list_tmp) + u[4], # x[5] is the y position of human
...
plot!(θ_samples, p1_mean_active_regret, ribbon=p1_var_passive_regret, xlabel="target θ", 
    ...
    linewidth=2)
savefig(plot_path*"final_hri_costs_p1_comparison.pdf")
...
savefig(plot_path*"final_hri_costs_p2_comparison.pdf")

...
costs = (FunctionPlayerCost((g, x, u, t) -> ( 10*(x[8]- 0.5)^2+2*(x[3]-pi/2)^2+ 2*(x[4]-0.7)^2 + u[1]^2 + u[2]^2 - 0.1*((x[1]-x[5])^2 + (x[2]-x[6])^2))),
         FunctionPlayerCost((g, x, u, t) -> (  2*(x[5] - 0.5)^2 +2*(x[7]-pi/2)^2 + 2*(x[8]-1)^2  + u[3]^2 + u[4]^2 - 0.1*((x[1]-x[5])^2 + (x[2]-x[6])^2))))
...
x0 = SVector(0.5, 0.2, pi/2, 1,       1, 0, pi/2, 1)
...
gif(anim1, "slowing_down_OL.gif", fps = 10)
...
gif(anim2, "slowing_down_FB.gif", fps = 10)

#--------------------------------------------------------------------------------------------------------------------------------

function parameterized_cost(θ::Vector)
    costs = (FunctionPlayerCost((g, x, u, t) -> ( θ[1]*(x[8]- 0.5)^2+θ[2]*((x[3]-pi/2)^2+ (x[4]-0.7)^2) + θ[3]*(u[1]^2 + u[2]^2) - θ[4]*((x[1]-x[5])^2 + (x[2]-x[6])^2))),
             FunctionPlayerCost((g, x, u, t) -> (  θ[5]*(x[5] - 0.5)^2 +θ[6]*((x[7]-pi/2)^2 + (x[8]-1)^2)  + θ[7]*(u[3]^2 + u[4]^2) - θ[8]*((x[1]-x[5])^2 + (x[2]-x[6])^2))))
    return costs
end
...
function inverse_game_gradient_descent(θ::Vector, g::GeneralGame, expert_traj::SystemTrajectory, x0::SVector, 
    ...
    for iter in 1:max_GD_iteration_num
        ...
        while minimum(θ_next)<=0 || θ_next[3]>1.1 || θ_next[6]>1.1
            # @infiltrate
            α = α*0.5^4
            θ_next = θ-α*gradient_value
        end

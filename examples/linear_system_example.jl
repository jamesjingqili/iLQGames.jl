...
# state: (px, py, phi, v)
dx(cs::DoubleUnicycle, x, u, t) = SVector(u[1],u[2],u[3],u[4])
...
loss_values[1],_,_ = inverse_game_loss(sol[1], g, expert_traj2, x0, parameterized_cost, "OLNE_costate")
...
for iter in 1:max_GD_iteration_num
    sol[iter+1], loss_values[iter+1], gradient[iter], equilibrium_type[iter], _, _ = inverse_game_gradient_descent(sol[iter], 
                                            g, expert_traj1, x0, 10, parameterized_cost, "FBNE_costate",true)

...
num_players=4
nx, nu, ΔT, game_horizon = 4*num_players, 2*num_players, 0.1, 50
...
col_coeff_1, col_coeff_2, col_coeff_3, col_coeff_4 = 1, 0.01, 0.01, 0.01
...
costs = (FunctionPlayerCost((g, x, u, t) -> (  4*(x[1]-1)^2  + 2*(x[3]-pi/2)^2+2*(x[4]-2)^2 +    2*(u[1]^2 + u[2]^2)    
                               -col_coeff_1*log((x[1]-x[3*4+1])^2+(x[2]-x[3*4+2])^2)) ),
         FunctionPlayerCost((g, x, u, t) -> (  2*(x[1*4+1])^2  + 2*(x[8]-1)^2 + 2*(u[3]^2 + u[4]^2) )    -col_coeff_2*log((x[4+1]-x[3*4+1])^2+(x[4+2]-x[3*4+2])^2)),
         FunctionPlayerCost((g, x, u, t) -> (  2*(x[2*4+1]-1)^2  + 2*(x[12]-1)^2 + 2*(u[5]^2 + u[6]^2) ) -col_coeff_3*log((x[2*4+1]-x[3*4+1])^2+(x[2*4+2]-x[3*4+2])^2)),
         FunctionPlayerCost((g, x, u, t) -> (  2*(x[3*4]-1)^2  + 2*(x[16]-1)^2 + 2*(u[7]^2 + u[8]^2) )-col_coeff_4*log((x[3*4+1]-x[2*4+1])^2+(x[3*4+2]-x[2*4+2])^2)) 
         )
...
x0 = SVector(0.5, 0, pi/2, 2,       0, 0, pi/2, 1,      1,2,pi/2,1,      0,3,pi/2,1)
...
function parameterized_cost(θ::Vector)
    costs = (FunctionPlayerCost((g, x, u, t) -> (  θ[1]*(x[1]-1)^2  + 2*(x[3]-pi/2)^2+2*(x[4]-2)^2 +    2*(u[1]^2 + u[2]^2)    
                               -col_coeff_1*log((x[1]-x[3*4+1])^2+(x[2]-x[3*4+2])^2)) ),
         FunctionPlayerCost((g, x, u, t) -> (  θ[2]*(x[1*4+1])^2  + 2*(x[8]-1)^2 + 2*(u[3]^2 + u[4]^2) )    -col_coeff_2*log((x[4+1]-x[3*4+1])^2+(x[4+2]-x[3*4+2])^2)),
         FunctionPlayerCost((g, x, u, t) -> (  2*(x[2*4+1]-1)^2  + 2*(x[12]-1)^2 + 2*(u[5]^2 + u[6]^2) ) -col_coeff_3*log((x[2*4+1]-x[3*4+1])^2+(x[2*4+2]-x[3*4+2])^2)),
         FunctionPlayerCost((g, x, u, t) -> (  2*(x[3*4]-1)^2  + 2*(x[16]-1)^2 + 2*(u[7]^2 + u[8]^2) )-col_coeff_4*log((x[3*4+1]-x[2*4+1])^2+(x[3*4+2]-x[2*4+2])^2)) 
         )    
    return costs
...
ForwardDiff.gradient(x -> loss(x, dynamics, "FBNE_costate", expert_traj2, true, false, [], [], 1:game_horizon-1, 1:nx, 1:nu, false, false, []), [4,4])

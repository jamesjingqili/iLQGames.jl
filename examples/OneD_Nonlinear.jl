...
nx, nu, ΔT, game_horizon = 1, 2, 0.1, 3
struct NLSystem <: ControlSystem{ΔT,nx,nu} end
dx(cs::NLSystem, x, u, t) = SVector(x[1]^2 + u[1] + u[2])
dynamics = NLSystem()
costs = (FunctionPlayerCost((g, x, u, t) -> (  20*(x[1])^2  + 2*u[1]^2  )),
         FunctionPlayerCost((g, x, u, t) -> (  4*(x[1])^2 + u[2]^2  )),
         )
...
function parameterized_cost(θ::Vector)
    costs = (FunctionPlayerCost((g, x, u, t) -> (  θ[1]*(x[1])^2  + 2*u[1]^2  )),
         FunctionPlayerCost((g, x, u, t) -> (  θ[2]*(x[1])^2 + u[2]^2  )),
         )
    return costs
end
θ_true=[4,4]

tmp1=loss([20,4], dynamics, "FBNE_costate", expert_traj2, false)
tmp2=loss([5,5], dynamics, "FBNE_costate", expert_traj2, false)

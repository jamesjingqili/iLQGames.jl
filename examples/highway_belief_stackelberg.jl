...
costs2 = (FunctionPlayerCost((g, x, u, t) -> (10*(x[5]-x[10])^2 + 2*(x[4]-1)^2  + u[1]^2 + u[2]^2 )), # target lane is x[10], mean of player 2 belief
...
x021 = SVector(0, 0.5, pi/2, 1,       
              1, 0, pi/2, 1,  
              0, 0, 1) # the last two states are: player 2's belief mean, player 2's belief variance
solver21 = iLQSolver(g21, max_scale_backtrack=5, max_n_iter=10, max_elwise_diff_step=Inf, equilibrium_type="Stackelberg_KKT_dynamic_factorization")
...
# TODO: debug the belief update: see whether we can converge!!!
...
π21_P_list = [ π21[t].P[3:4,:] for t in 1:game_horizon ]
push!(π21_P_list, π21_P_list[end])
π21_α_list = [ π21[t].α[3:4,:] for t in 1:game_horizon ]

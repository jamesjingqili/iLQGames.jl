...
x12_FB, y12_FB = [x1.x[i][4] for i in 1:game_horizon], [x1.x[i][5] for i in 1:game_horizon];
...
savefig("assistive_robot_step1.png")
...
(u_list[Int(floor(t/ΔT))+1][2] - π1_P_list[Int(floor(t/ΔT))+1][2,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π1_α_list[Int(floor(t/ΔT))+1][2]-u[4])*cos(x[3]) + (u_list[Int(floor(t/ΔT))+1][1] - π1_P_list[Int(floor(t/ΔT))+1][1,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π1_α_list[Int(floor(t/ΔT))+1][1]-u[3])*sin(x[3]), # x[3] is the heading angle of the robot
-1/ΔT*x[4] + 1/ΔT*x[1] + 1/ΔT * L*cos(x[3]) + u[3], # x[4] is the x position of human
-1/ΔT*x[5] + 1/ΔT*x[2] + 1/ΔT * L*sin(x[3]) + u[4], # x[5] is the y position of human
0, # x[6] is the ground truth state 
- 1/ΔT* x[end]*π1_P_list[Int(floor(t/ΔT))+1][3:4,7]'*inv(I(2)+π1_P_list[Int(floor(t/ΔT))+1][3:4,7]*x[end]*π1_P_list[Int(floor(t/ΔT))+1][3:4,7]') * ( 
...
- 1/ΔT* x[end]*π1_P_list[Int(floor(t/ΔT))+1][3:4,7]'*inv(I(2)+π1_P_list[Int(floor(t/ΔT))+1][3:4,7]*x[end]*π1_P_list[Int(floor(t/ΔT))+1][3:4,7]') * 
π1_P_list[Int(floor(t/ΔT))+1][3:4,7]*x[end]                 # variance
) + SVector(  
...
costs2 = (
    FunctionPlayerCost((g, x, u, t) -> ( (x[1])^2 + (x[2])^2 +  10*(x[3] - x[7])^2 + (u[1])^2 + (u[2])^2 + σ1*(
        (u_list[Int(floor(t/ΔT))+1][1] - π1_P_list[Int(floor(t/ΔT))+1][1,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π1_α_list[Int(floor(t/ΔT))+1][1])^2 + 
        (u_list[Int(floor(t/ΔT))+1][2] - π1_P_list[Int(floor(t/ΔT))+1][2,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π1_α_list[Int(floor(t/ΔT))+1][2])^2) )),  # robot cost
    FunctionPlayerCost((g, x, u, t) -> ( 10*(x[3] - x[6])^2 + σ2*((
        u[3]-(u_list[Int(floor(t/ΔT))+1][1] - π1_P_list[Int(floor(t/ΔT))+1][1,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π1_α_list[Int(floor(t/ΔT))+1][1])
        )^2 + (
            u[4]-(u_list[Int(floor(t/ΔT))+1][2] - π1_P_list[Int(floor(t/ΔT))+1][2,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π1_α_list[Int(floor(t/ΔT))+1][2])
            )^2) )) # human cost
); 
...
savefig("assistive_robot_step2.png")
...
dx(cs::player_dynamics3, x, u, t) = SVector(
    ...
    -1/ΔT*x[5] + 1/ΔT*x[2] + 1/ΔT * L*sin(x[3]) + u[4], # x[5] is the y position of human
    0, 
    -1/ΔT* x[end]*π2_P_list[Int(floor(t/ΔT))+1][3:4,7]'*inv(I(2)+π2_P_list[Int(floor(t/ΔT))+1][3:4,7]*x[end]*π2_P_list[Int(floor(t/ΔT))+1][3:4,7]') * ( 
        [u[3];u[4]] - u2_list[Int(floor(t/ΔT))+1][3:4]+ 
        π2_P_list[Int(floor(t/ΔT))+1][3:4,:]*(x-x2_list[Int(floor(t/ΔT))+1]) + π2_α_list[Int(floor(t/ΔT))+1][3:4] ),  # mean 
    - 1/ΔT* x[end]*π2_P_list[Int(floor(t/ΔT))+1][3:4,7]'*inv(I(2)+π2_P_list[Int(floor(t/ΔT))+1][3:4,7]*x[end]*π2_P_list[Int(floor(t/ΔT))+1][3:4,7]') * 
    π2_P_list[Int(floor(t/ΔT))+1][3:4,7]*x[end]                 # variance
);
...
0, # x[1] is the x position of the robot
0, # x[2] is the y position of the robot
(u_list[Int(floor(t/ΔT))+1][2] - π3_P_list[Int(floor(t/ΔT))+1][2,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π3_α_list[Int(floor(t/ΔT))+1][2]-u[4])*cos(x[3]) + (u_list[Int(floor(t/ΔT))+1][1] - π3_P_list[Int(floor(t/ΔT))+1][1,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π3_α_list[Int(floor(t/ΔT))+1][1]-u[3])*sin(x[3]), # x[3] is the heading angle of the robot
-1/ΔT*x[4] + 1/ΔT*x[1] + 1/ΔT * L*cos(x[3]) + u[3], # x[4] is the x position of human
-1/ΔT*x[5] + 1/ΔT*x[2] + 1/ΔT * L*sin(x[3]) + u[4], # x[5] is the y position of human
0, # x[6] is the ground truth state 
- 1/ΔT* x[end]*π3_P_list[Int(floor(t/ΔT))+1][3:4,7]'*inv(I(2)+π3_P_list[Int(floor(t/ΔT))+1][3:4,7]*x[end]*π3_P_list[Int(floor(t/ΔT))+1][3:4,7]') * ( 
    [u[3];u[4]] - u_list[Int(floor(t/ΔT))+1][3:4]+ 
    π3_P_list[Int(floor(t/ΔT))+1][3:4,:]*(x-x_list[Int(floor(t/ΔT))+1]) + π3_α_list[Int(floor(t/ΔT))+1][3:4] ),  # mean 
- 1/ΔT* x[end]*π3_P_list[Int(floor(t/ΔT))+1][3:4,7]'*inv(I(2)+π3_P_list[Int(floor(t/ΔT))+1][3:4,7]*x[end]*π3_P_list[Int(floor(t/ΔT))+1][3:4,7]') * 
    π3_P_list[Int(floor(t/ΔT))+1][3:4,7]*x[end]                 # variance
) + SVector(  
u_list[Int(floor(t/ΔT))+1][1] - π3_P_list[Int(floor(t/ΔT))+1][1,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π3_α_list[Int(floor(t/ΔT))+1][1], 
u_list[Int(floor(t/ΔT))+1][2] - π3_P_list[Int(floor(t/ΔT))+1][2,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π3_α_list[Int(floor(t/ΔT))+1][2],
0,0,0,
0,0,0
);
...
costs4 = (
    FunctionPlayerCost((g, x, u, t) -> ( (x[1])^2 + (x[2])^2 +  10*(x[3] - x[7])^2 + (u[1])^2 + (u[2])^2 + σ1*(
        (u_list[Int(floor(t/ΔT))+1][1] - π3_P_list[Int(floor(t/ΔT))+1][1,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π3_α_list[Int(floor(t/ΔT))+1][1])^2 + 
        (u_list[Int(floor(t/ΔT))+1][2] - π3_P_list[Int(floor(t/ΔT))+1][2,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π3_α_list[Int(floor(t/ΔT))+1][2])^2) )),  # robot cost
    FunctionPlayerCost((g, x, u, t) -> ( 10*(x[3] - x[6])^2 + σ2*((
        u[3]-(u_list[Int(floor(t/ΔT))+1][1] - π3_P_list[Int(floor(t/ΔT))+1][1,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π3_α_list[Int(floor(t/ΔT))+1][1])
        )^2 + (
            u[4]-(u_list[Int(floor(t/ΔT))+1][2] - π3_P_list[Int(floor(t/ΔT))+1][2,:]'*(x-x_list[Int(floor(t/ΔT))+1])-π3_α_list[Int(floor(t/ΔT))+1][2])
            )^2) )) # human cost
); 
...
belief_list = [ x2.x[t][7] for t in 1:game_horizon ]
var_list = [x2.x[t][end] for t in 1:game_horizon]
x4_costs_player_1 = sum([costs4[1](g4, x2.x[t], x2.u[t], t*ΔT)[1] for t in 1:game_horizon])
x4_costs_player_2 = sum([costs4[2](g4, x2.x[t], x2.u[t], t*ΔT)[1] for t in 1:game_horizon])
# step 4 finished!
...
x11_FB, y11_FB = [x2.x[i][1] for i in 1:game_horizon], [x2.x[i][2] for i in 1:game_horizon];
x12_FB, y12_FB = [x2.x[i][4] for i in 1:game_horizon], [x2.x[i][5] for i in 1:game_horizon];
...
savefig("assistive_robot_old.png")

savefig("assistive_robot_steps1.png")

savefig("assistive_robot_steps.png")


plot(1:game_horizon, [x2_list[t][3] for t in 1:game_horizon], xlabel="time", ylabel="theta",label="")
hline!([θ],label="target")

savefig("assistive_robot_theta_old.png")


savefig("assistive_robot_theta.png")
...
savefig("assistive_robot_belief_old.png")

savefig("assistive_robot_belief1.png")

savefig("assistive_robot_belief.png")
...
#     0, # x[1] is the x position of the robot
#     0, # x[2] is the y position of the robot
#     (    u3_list[Int(floor(t/ΔT))+1][2] - π3_P_list[Int(floor(t/ΔT))+1][2,:]'*(x-x3_list[Int(floor(t/ΔT))+1])-π3_α_list[Int(floor(t/ΔT))+1][2]-u[4])*cos(x[3]) + (u3_list[Int(floor(t/ΔT))+1][1] - π3_P_list[Int(floor(t/ΔT))+1][1,:]'*(x-x3_list[Int(floor(t/ΔT))+1])-π3_α_list[Int(floor(t/ΔT))+1][1]-u[3])*sin(x[3]), # x[3] is the heading angle of the robot
#     -1/ΔT*x[4] + 1/ΔT*x[1] + 1/ΔT * L*cos(x[3]) + u[3], # x[4] is the x position of human
#     -1/ΔT*x[5] + 1/ΔT*x[2] + 1/ΔT * L*sin(x[3]) + u[4], # x[5] is the y position of human
#     0, 
#     - 1/ΔT* x[end]*π3_P_list[Int(floor(t/ΔT))+1][3:4,7]'*inv(I(2)+π3_P_list[Int(floor(t/ΔT))+1][3:4,7]*x[end]*π3_P_list[Int(floor(t/ΔT))+1][3:4,7]') * ( 
#         [u[3];u[4]] - u3_list[Int(floor(t/ΔT))+1][3:4]+ 
#         π3_P_list[Int(floor(t/ΔT))+1][3:4,:]*(x-x3_list[Int(floor(t/ΔT))+1]) + π3_α_list[Int(floor(t/ΔT))+1][3:4] ),  # mean 
#     - 1/ΔT* x[end]*π3_P_list[Int(floor(t/ΔT))+1][3:4,7]'*inv(I(2)+π3_P_list[Int(floor(t/ΔT))+1][3:4,7]*x[end]*π3_P_list[Int(floor(t/ΔT))+1][3:4,7]') * 
#     π3_P_list[Int(floor(t/ΔT))+1][3:4,7]*x[end]                 # variance
#     ) + SVector(     
...
# dynamics4 = player_dynamics4();
# costs4 = (
#     FunctionPlayerCost((g, x, u, t) -> ( (x[1])^2 + (x[2])^2 + 10* (x[3] - x[7])^2 + σ1*(
#         (u3_list[Int(floor(t/ΔT))+1][1] - π3_P_list[Int(floor(t/ΔT))+1][1,:]'*(x-x3_list[Int(floor(t/ΔT))+1])-π3_α_list[Int(floor(t/ΔT))+1][1])^2 + 
#         (u3_list[Int(floor(t/ΔT))+1][2] - π3_P_list[Int(floor(t/ΔT))+1][2,:]'*(x-x3_list[Int(floor(t/ΔT))+1])-π3_α_list[Int(floor(t/ΔT))+1][2])^2
#     ) )),  # robot cost
#     FunctionPlayerCost((g, x, u, t) -> ( 10*(x[3] - x[6])^2 + σ2*((
#         u[3]-(u3_list[Int(floor(t/ΔT))+1][1] - π3_P_list[Int(floor(t/ΔT))+1][1,:]'*(x-x3_list[Int(floor(t/ΔT))+1])-π3_α_list[Int(floor(t/ΔT))+1][1])
#         )^2 + (u[4]-(u3_list[Int(floor(t/ΔT))+1][2] - π3_P_list[Int(floor(t/ΔT))+1][2,:]'*(x-x3_list[Int(floor(t/ΔT))+1])-π3_α_list[Int(floor(t/ΔT))+1][2])
#         )^2) )) # human cost
# );
# g4 = GeneralGame(game_horizon, player_inputs, dynamics4, costs4);
# solver4 = iLQSolver(g4, max_scale_backtrack=5, max_n_iter=10, max_elwise_diff_step=Inf, equilibrium_type="FBNE");

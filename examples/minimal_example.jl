...
costs = (FunctionPlayerCost((g, x, u, t) -> (10*(x[1]-1)^2 + 0.1*(x[3]-pi/2)^2 + (x[4]-1)^2 + 10*u[1]^2 + u[2]^2 - 0.1*((x[1]-x[5])^2 + (x[2]-x[6])^2))),
         FunctionPlayerCost((g, x, u, t) -> ((x[5]-1)^2 + 0.1*(x[7]-pi/2)^2 + (x[8]-1)^2 + 10*u[3]^2 + u[4]^2- 0.1*((x[1]-x[5])^2 + (x[2]-x[6])^2))))
...
solver1 = iLQSolver(g, max_n_iter=500, equilibrium_type="OLNE")
x0 = SVector(0, 0, pi/2, 1,       1, 0, pi/2, 1)
@time converged1, trajectory1, strategies1 = solve(g, solver1, x0)

solver2 = iLQSolver(g, max_n_iter=500, equilibrium_type="FBNE")
x0 = SVector(0, 0, pi/2, 1,       1, 0, pi/2, 1)
@time converged2, trajectory2, strategies2 = solve(g, solver2, x0)

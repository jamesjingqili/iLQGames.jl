...
nx, nu, ΔT, game_horizon = 6, 2, 0.1, 10
...
initial_belief = 0.5
initial_state = SVector(-5, 1.5, -2,1)
initial_state_truth = vcat(initial_state, SVector(θ,θ))
...
costs = (FunctionPlayerCost((g, x, u, t) -> ( 2*x[5]*(x[1]-10)^2 - log(x[3]-x[1]) +4*(x[2]-initial_state[2])^2+ u[1]^2 )), # target lane is x[10], mean of player 2 belief
          FunctionPlayerCost((g, x, u, t) -> ( -log(x[3]-x[1])+ u[2]^2+ 2*relu(1.5-(x[3]+1)^2) ))) 
...
x1_FB, x2_FB = [x_true.x[i][1] for i in 1:game_horizon], [x_true.x[i][3] for i in 1:game_horizon];
...
π_P_list = [ π_true[t].P for t in 1:game_horizon ]
push!(π_P_list, π_P_list[end])
π_α_list = [ π_true[t].α for t in 1:game_horizon ]
...
dx(cs::player_dynamics1, x, u, t) = SVector(  
  ...
  x[4],
  u[2],
  0,  # parameter of player 1, invisible to player 2, binary variable
...
x11_FB = [x1.x[i][1] for i in 1:game_horizon];
x12_FB = [x1.x[i][3] for i in 1:game_horizon];
...
# x02 = vcat(initial_state, SVector(θ,initial_belief))


struct player_dynamics2 <: ControlSystem{ΔT, 6, 2 } end
...
0,
0,
1/ΔT*(pdf.(Normal(u_list[Int(floor(t/ΔT))+1][1]-π1_P_list[Int(floor(t/ΔT))+1][1,[1,2,3,4,5]]'*(vcat(x[[1,2,3,4]], 1)-x_list[Int(floor(t/ΔT))+1][[1,2,3,4,5]])-π1_α_list[Int(floor(t/ΔT))+1][1], 2), u[1])*x[6])/(pdf.(
        Normal(u_list[Int(floor(t/ΔT))+1][1]-π1_P_list[Int(floor(t/ΔT))+1][1,[1,2,3,4,5]]'*(vcat(x[[1,2,3,4]], 1)-x_list[Int(floor(t/ΔT))+1][[1,2,3,4,5]])-π1_α_list[Int(floor(t/ΔT))+1][1], 2), u[1])*x[6] +
  pdf.(Normal(u_list[Int(floor(t/ΔT))+1][1]-π1_P_list[Int(floor(t/ΔT))+1][1,[1,2,3,4,5]]'*(vcat(x[[1,2,3,4]], 0)-x_list[Int(floor(t/ΔT))+1][[1,2,3,4,5]])-π1_α_list[Int(floor(t/ΔT))+1][1], 2), u[1])*(1-x[6])) - 1/ΔT * x[6]
) + SVector(0,0,     0,
...
# costs2 = (FunctionPlayerCost((g, x, u, t) -> ( x[5]*(x[1]-10)^2 - log(x[3]-x[1]) + u[1]^2 )), # target lane is x[10], mean of player 2 belief
# FunctionPlayerCost((g, x, u, t) -> ( -log(x[3]-x[1])+ u[2]^2+ 20*relu(1.5-(x[3]+1)^2) ))) 
...
belief_list = [ x2.x[t][6] for t in 1:game_horizon ]
# step 2 finished!




x21_FB = [x2.x[i][1] for i in 1:game_horizon];
x22_FB = [x2.x[i][3] for i in 1:game_horizon];

scatter(x21_FB, 0*x1_FB,markersize=6*marker_list,label="player 1")
scatter!(x22_FB, 0*x2_FB,markersize=6*marker_list,label="player 2")
title!("chill")
...
# x22_FB, y22_FB = [x2.x[i][3] for i in 1:game_horizon], [x2.x[i][6] for i in 1:game_horizon];
...
struct player_dynamics3 <: ControlSystem{ΔT, 6, 2 } end
...
u[2],
0,
1/ΔT*(pdf.(Normal(u2_list[Int(floor(t/ΔT))+1][1]-π2_P_list[Int(floor(t/ΔT))+1][1,[1,2,3,4,5]]'*(vcat(x[[1,2,3,4]], 1)-x2_list[Int(floor(t/ΔT))+1][[1,2,3,4,5]])-π2_α_list[Int(floor(t/ΔT))+1][1], 2), u[1])*x[6])/(pdf.(
        Normal(u2_list[Int(floor(t/ΔT))+1][1]-π2_P_list[Int(floor(t/ΔT))+1][1,[1,2,3,4,5]]'*(vcat(x[[1,2,3,4]], 1)-x2_list[Int(floor(t/ΔT))+1][[1,2,3,4,5]])-π2_α_list[Int(floor(t/ΔT))+1][1], 2), u[1])*x[6] +
  pdf.(Normal(u2_list[Int(floor(t/ΔT))+1][1]-π2_P_list[Int(floor(t/ΔT))+1][1,[1,2,3,4,5]]'*(vcat(x[[1,2,3,4]], 0)-x2_list[Int(floor(t/ΔT))+1][[1,2,3,4,5]])-π2_α_list[Int(floor(t/ΔT))+1][1], 2), u[1])*(1-x[6])) - 1/ΔT * x[6]
) #+ SVector(0,0,     0,
#u2_list[Int(floor(t/ΔT))+1][2] - π2_P_list[Int(floor(t/ΔT))+1][2,:]'*(x-x2_list[Int(floor(t/ΔT))+1])-π2_α_list[Int(floor(t/ΔT))+1][2], 
#0,0 );
dynamics3 = player_dynamics3();

# costs3 = (FunctionPlayerCost((g, x, u, t) -> (  2*x[5]*(x[1]-10)^2 - log(x[3]-x[1]) + u[1]^2 )), # target lane is x[10], mean of player 2 belief
#           FunctionPlayerCost((g, x, u, t) -> ( -log(x[3]-x[1])+ u[2]^2+ 2*relu(1.5-(x[3]+1)^2) ))) 
...
belief_list = [ x3.x[t][6] for t in 1:game_horizon ]
...
struct player_dynamics4 <: ControlSystem{ΔT, 6, 2 } end
dx(cs::player_dynamics4, x, u, t) = SVector(
  ...
  0,
  0,
  1/ΔT*(pdf.(Normal(u3_list[Int(floor(t/ΔT))+1][1]-π3_P_list[Int(floor(t/ΔT))+1][1,[1,2,3,4,5]]'*(vcat(x[[1,2,3,4]], 1)-x3_list[Int(floor(t/ΔT))+1][[1,2,3,4,5]])-π3_α_list[Int(floor(t/ΔT))+1][1], 2), u[1])*x[6])/(
    pdf.(Normal(u3_list[Int(floor(t/ΔT))+1][1]-π3_P_list[Int(floor(t/ΔT))+1][1,[1,2,3,4,5]]'*(vcat(x[[1,2,3,4]], 1)-x3_list[Int(floor(t/ΔT))+1][[1,2,3,4,5]])-π3_α_list[Int(floor(t/ΔT))+1][1], 2), u[1])*x[6] +
    pdf.(Normal(u3_list[Int(floor(t/ΔT))+1][1]-π3_P_list[Int(floor(t/ΔT))+1][1,[1,2,3,4,5]]'*(vcat(x[[1,2,3,4]], 0)-x3_list[Int(floor(t/ΔT))+1][[1,2,3,4,5]])-π3_α_list[Int(floor(t/ΔT))+1][1], 2), u[1])*(1-x[6])) - 1/ΔT * x[6]
  ) + SVector(0,0,     0,
  u3_list[Int(floor(t/ΔT))+1][2] - π3_P_list[Int(floor(t/ΔT))+1][2,1:5]'*(x[[1,2,3,4,6]]-x3_list[Int(floor(t/ΔT))+1][[1,2,3,4,5]])-π3_α_list[Int(floor(t/ΔT))+1][2], 
  0,0 );
dynamics4 = player_dynamics4();
# costs4 = (FunctionPlayerCost((g, x, u, t) -> ( x[5]*(x[1]-10)^2 - log(x[3]-x[1]) + u[1]^2 )), # target lane is x[10], mean of player 2 belief
#   FunctionPlayerCost((g, x, u, t) -> ( -log(x[3]-x[1])+ u[2]^2+ 20*relu(1.5-(x[3]+1)^2) )))  
g4 = GeneralGame(game_horizon, player_inputs, dynamics4, costs);
...
belief_list = [ x4.x[t][6] for t in 1:game_horizon ]
# var_list = [x4.x[t][11] for t in 1:game_horizon]
# step 4 finished!



x1_FB = [x4.x[i][1] for i in 1:game_horizon]
x2_FB = [x4.x[i][3] for i in 1:game_horizon]

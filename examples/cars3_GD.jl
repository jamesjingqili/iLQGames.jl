...
col_coeff_1, col_coeff_2 = 1, 0.001
...
costs = (FunctionPlayerCost((g, x, u, t) -> (  2*(x[1])^2  + 2*(x[3]-pi/2)^2+8*(x[4]-2)^2 +    2*(u[1]^2 + u[2]^2)    
                               -col_coeff_1*(log((x[1]-x[4+1])^2+(x[2]-x[4+2])^2) + log((x[1]-x[2*4+1])^2 + (x[2]-x[2*4+2])^2))) ),
         FunctionPlayerCost((g, x, u, t) -> (  2*(x[1*4+3]-pi/2)^2  + 2*(x[8]-1.5)^2 + 2*(u[3]^2 + u[4]^2) )    -col_coeff_2*log((x[4+1]-x[1])^2+(x[4+2]-x[2])^2)),
         FunctionPlayerCost((g, x, u, t) -> (  2*(x[2*4+1])^2+2*(u[5]^2 + u[6]^2) )) ,
         )

player_inputs = (SVector(1,2), SVector(3,4), SVector(5,6))
g = GeneralGame(game_horizon, player_inputs, dynamics, costs)
# get a solver, choose initial conditions and solve (in about 9 ms with AD)
x0 = SVector(0.4, 0, pi/2, 2,       0.5, 2, pi/2, 1,      0,3,pi/2,1)
...
function parameterized_cost(θ::Vector)
    costs = (FunctionPlayerCost((g, x, u, t) -> (  θ[1]*(x[1])^2  + 2*(x[3]-pi/2)^2+θ[2]*(x[4]-2)^2 +    2*(u[1]^2 + u[2]^2)    
                               -col_coeff_1*(log((x[1]-x[4+1])^2+(x[2]-x[4+2])^2) + log((x[1]-x[2*4+1])^2 + (x[2]-x[2*4+2])^2))) ),
         FunctionPlayerCost((g, x, u, t) -> (  2*(x[1*4+3]-pi/2)^2  + θ[3]*(x[8]-1.5)^2 + 2*(u[3]^2 + u[4]^2) )   -col_coeff_2*log((x[4+1]-x[1])^2+(x[4+2]-x[2])^2)),
         FunctionPlayerCost((g, x, u, t) -> (  2*(x[2*4+1])^2+2*(u[5]^2 + u[6]^2) )) ,
         )
    return costs
end

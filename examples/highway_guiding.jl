...
function parameterized_cost(θ::Vector)
    costs = (FunctionPlayerCost((g, x, u, t) -> ( θ[1]*(x[5]-1)^2 + θ[2]*(x[4]-1)^2 + θ[3]*(u[1]^2 + u[2]^2) - 0*((x[1]-x[5])^2 + (x[2]-x[6])^2))),
             FunctionPlayerCost((g, x, u, t) -> (  θ[4]*(x[5] - x[1])^2 + θ[5]*(x[8]-1)^2 + θ[6]*(u[3]^2 + u[4]^2) - 0*((x[1]-x[5])^2 + (x[2]-x[6])^2))))
    return costs
end
...
selected_item = 40
loss_test1, traj_test1, _, _=loss(sol[selected_item], equilibrium_type[selected_item], noisy_expert_traj2, false)

...
for ii in 1:num_clean_traj
    for jj in 1:num_noise_level
        ...
        for kk in 1:num_obs
            tmp = zeros(num_test)
            tmp_ground_truth_loss[kk] = loss(θ_list_list[ii][jj][1][kk], iLQGames.dynamics(g), "FBNE_costate", expert_traj_list[ii], true,false, [],[],1:g.h-1, 1:12, 1:nu) # obs_state_list does not include [13]

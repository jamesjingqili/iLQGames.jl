...
solver = iLQSolver(g, max_scale_backtrack=5,max_n_iter=10, max_elwise_diff_step=Inf, 
equilibrium_type="Stackelberg_KKT_dynamic_factorization")
@time c, expert_traj, strategies = solve(g, solver, x0)


# solver = iLQSolver(g, max_scale_backtrack=20,max_n_iter=10, max_elwise_diff_step=Inf, 
# equilibrium_type="FBNE_KKT")
# @time c, expert_traj, strategies = solve(g, solver, x0)
...
anim = @animate for i in 1:game_horizon
    plot([x1_FB[i], x1_FB[i]], [y1_FB[i], y1_FB[i]], markershape = :square, label = "player 1, Stackelberg KKT", 
    xlims = (-0.5, 1.5), ylims = (0, 6))
    plot!([x2_FB[i], x2_FB[i]], [y2_FB[i], y2_FB[i]], markershape = :square, label = "player 2, Stackelberg KKT", 
    xlims = (-0.5, 1.5), ylims = (0, 6))
